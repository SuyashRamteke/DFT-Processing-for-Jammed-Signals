function NS_21_DFT_Relations(Fs,N,N_zp)
%%% A routnie showing CTFT, DTFT_inf, DTFT_N, and DFT Relations
%
...
ylabel('|CTFT(f)|')
axis_vals=axis;
Freq_Lims=[-3 3]*(Fs/2)/1000;
axis([Freq_Lims 0 2.5e-4]);
legend('CTFT')
title(['F_s = ' num2str(Fs/1000) 'kHz   N_{samples} = ' num2str(N) '   N_{zp} = ' num2str(N_zp)])
...
axis([-3 3  0 2.5e-4])
hold off
legend('DTFT_{Inf}','CTFT')

%%%%  Compute the Theoretical DTFT_N
DTFT_N=(1-(a*exp(-j*omega)).^N)./(1 - a*exp(-j*omega));    %%% from Eq. (7.6) in Kamen & Heck 
subplot(4,1,3)
h=plot(omega/pi,abs(T*DTFT_inf));
set(h,'linewidth',2);
set(gca,'fontsize',12);
